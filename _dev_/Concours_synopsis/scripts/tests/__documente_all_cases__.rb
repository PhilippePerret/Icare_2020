...
class << self
  ...
  def build_documentation
    ...
    stream << "</dl>\n"
    stream << footer
    puts "La documentation a été établie (dans le fichier '_it_cases_/__all_it-cases__.html')"
  ensure
...
<em>Pour actualiser cette liste, lancer le script '#{__FILE__}' et recharger cette page</em>
  ...
  def path_documentation
    @path_documentation ||= File.join(FOLDER_ITCASES,'__all_it-cases__.html')
  end
...
FOLDER_ITCASES = './spec/features/concours/per_people/_it_cases_'
Dir["#{FOLDER_ITCASES}/**/*.rb"].each do |fpath|
