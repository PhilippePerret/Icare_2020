...
class << self
  def proceed_test
    ...
    seulement_concours = only_route && only_route == 'concours'
    ...
    puts "=== OPTIONS ===".bleu
    [
      seulement_concours ? "Seulement la route : #{only_route}" : (option?(:concours) ? "Seulement le CONCOURS (la phase courante)" : "Toutes les routes"),
    ...
    # Sauf si on ne demande le test que d'une seule route, on teste
    # le concours suivant son état courant.
    CheckedUrl.phase_concours unless only_route && not(seulement_concours)
    puts RC * 2
