  ...
  def description_etape_courante
    t = case Concours.current.phase
        ...
        when 1 then
          ...
          segs.join(" ")
        when 2 then "<strong>Les #{nombre_synopsis} synopsis sont en cours de présélection</strong>.<br/><br/>Rendez-vous aux alentours du #{date_premiere_selection} pour les résultats de la première sélection !"
        when 3 then "<img class=\"emoji fleft center-on-mobile\" src=\"./img/Emojis/objets/coupe/coupe-big.png\" /><div class=\"clear-on-mobile\"><strong>Les 10 synopsis sélectionnés<br/>sont en finale</strong>.<br/><br/>#{PALMARES_LINK.with('☞ Voir le palmarès actuel ☜')}<br/><br/>Rendez-vous aux alentours<br/>du #{date_selection_finale}<br/>pour le palmarès final.</div>"
