...
class Operation
  def send_mail_icariens_annonce_start(options)
    ...
    MailSender.send_mailing({from:CONCOURS_MAIL, to:db_exec(request), file:mail_path('phase1/annonce_start_icariens'), bind:self}, options)
  end

  def send_mail_concurrents_annonce_start(options)
    request = "SELECT patronyme AS pseudo, mail, sexe FROM #{DBTBL_CONCURRENTS}"
    MailSender.send_mailing({from:CONCOURS_MAIL, to:db_exec(request), file:mail_path('phase1/annonce_start_concurrents'), bind:self}, options)
  end

  def send_mail_jury_annonce_start(options)
    MailSender.send_mailing({from:CONCOURS_MAIL, to:Concours.current.jury_members, file:mail_path('phase1/annonce_start_jury'), bind:self}, options)
  end
