...
def peut_modifier_ses_preferences_notifications
  it "peut modifier ses préférences pour le mail d'infos" do
    ...
    expect(vtested.options[0]).to eq("1"), "Les options du concurrent devraient être réglées pour recevoir le mail d'info (c'est un problème qu'il faut régler au niveau du test)"
    ...
    # *** Test ***
    vtested.click_on(LINK_DONT_WANT_MAIL_INFOS)
    # *** Vérifications ***
    ...
    vtested.reset
    expect(vtested.options[0]).to eq("0")
    # *** Test 2***
    vtested.click_on(LINK_WANT_MAIL_INFOS)
    # *** Vérifications ***
    ...
    # Dans les données du concurrent
    vtested.reset
    expect(vtested.options[0]).to eq("1")
    vtested.logout
  end
...
def peut_modifier_ses_preferences_fiche_de_lecture
  it "peut modifier ses préférences de fiche de lecture" do
    vtested = visitor.is_a?(TUser) ? visitor.as_concurrent : visitor
    expect(vtested.options[1]).to eq("1"), "Les options du concurrent devraient être réglées pour recevoir la fiche de lecture (c'est un problème qu'il faut régler au niveau du test)"
    goto("concours/espace_concurrent")
    expect(page).to be_espace_personnel
    expect(page).to have_content(MESSAGE_RECOIT_FICHE_LECTURE)
    expect(page).to have_link(LINK_DONT_WANT_FICHE_LECTURE)
    # *** Test ***
    vtested.click_on(LINK_DONT_WANT_FICHE_LECTURE)
    # *** Vérifications ***
    ...
    # Dans les données du concurrent
    vtested.reset
    expect(vtested.options[1]).to eq("0")
    # *** Test 2***
    vtested.click_on(LINK_WANT_FICHE_LECTURE)
    # *** Vérifications ***
    ...
    # Dans les données du concurrent
    vtested.reset
    expect(vtested.options[1]).to eq("1")
  end
