...
describe 'Le runner du cronjob' do
  ...
  it 'le nettoyage des mails se font seulement le samedi, à 1 heure' do
    ...
    expect(code).to include "RUN [nettoyage_dossiers]"
  end
  ...
  it 'le nettoyage des dossiers se fait seulement le samedi, à 2 heures' do
    ...
    code = File.read(MAIN_LOG_PATH)
    expect(code).not_to include "RUN [nettoyage_dossiers]"
    expect(code).to include "NOT TIME FOR [nettoyage_mails]"
  end
  ...
  it 'l’envoi des actualités se fait tous les jours, à 3 heures' do
    [
      ...
      code = File.read(MAIN_LOG_PATH)
      expect(code).not_to include "NOT TIME FOR [envoi_actualites]"
      expect(code).to include "NOT TIME FOR [nettoyage_dossiers]"
      expect(code).to include "NOT TIME FOR [nettoyage_mails]"
    end
    [
      "2020/10/24/1/0",
